...
local cachedFontName = "SystemBold"
local function getEspFont()
local name = cfg.settings.fontName
if name == cachedFontName then return cachedFont end
cachedFontName = name
cachedFont = FONT_MAP[name] or Drawing.Fonts.SystemBold
return cachedFont
end
local STR_PANIC     = "*PANIC*"
local STR_STOLEN    = "*Stolen Vehicle*"
local STR_HP_PREFIX = "HP: "
local STR_DOLLAR    = "$"
local STR_SPOTLIGHT = "Spotlighted: "
local GUI_OFF = {
Visible          = 1453,
Text             = 3576,
...
pcall(function() cachedMouse = LocalPlayer and LocalPlayer:GetMouse() end)
end
return cachedMouse
end
local cachedPG, cachedMenus, pgCacheT = nil, nil, 0
local function getCachedMenus()
local now = os_clock()
if now - pgCacheT > 0.1 then
pgCacheT = now
pcall(function()
cachedPG = LocalPlayer:FindFirstChildOfClass("PlayerGui")
cachedMenus = cachedPG and cachedPG:FindFirstChild("GameMenus")
end)
end
return cachedMenus
end
local function getPlayerGui()
if cachedPG and cachedPG.Parent then return cachedPG end
pcall(function()
cachedPG = LocalPlayer:FindFirstChildOfClass("PlayerGui")
end)
return cachedPG
end
local _rect = { x = 0, y = 0, w = 0, h = 0, cx = 0, cy = 0 }
...
_rect.x, _rect.y, _rect.w, _rect.h = x, y, w, h
_rect.cx, _rect.cy = x + w * 0.5, y + h * 0.5
return _rect
end
...
return ry < vh - pad and (ry + rh) > pad and rx < vw - pad and (rx + rw) > pad
end
local function moveMouseToward(tx, ty)
local vw, vh = viewportSize()
if vw then
tx = math_clamp(tx, 8, vw - 8)
ty = math_clamp(ty, 8, vh - 8)
end
local mx, my = mouseXY()
...
local dy = ty - my
if vw then
dx = math_clamp(mx + dx, 8, vw - 8) - mx
dy = math_clamp(my + dy, 8, vh - 8) - my
end
...
local cx, cy = rect.cx, rect.cy
local dist = moveMouseToward(cx, cy)
...
local distNorm = math_min((tonumber(dist) or 0) / DYNAMIC_REF_DIST, 1)
...
return math_max(8, math_floor(baseSize * scale + 0.5))
end
local function applyTextStyle(label, fs)
if not label then return end
fs = math_max(8, math_floor(tonumber(fs) or 10))
if label._cFS == fs and label._cFN == cachedFontName then return end
label._cFS = fs
label._cFN = cachedFontName
local font = getEspFont()
label.Font = font
label.Size = fs
end
local function createTextEsp(size)
local label = DrawNew("Text")
label.Center  = true
label.Outline = true
label.ZIndex  = 120
label.Visible = false
...
local MAX_HEALTH_NAMES = { "MaxHealth", "Max_Health", "maxHealth", "HealthMax", "MaxHP" }
local function getVehicleMaxHealth(model)
if not model then return nil end
local cv = model:FindFirstChild("Control_Values")
if not cv then return nil end
for i = 1, #MAX_HEALTH_NAMES do
local obj = cv:FindFirstChild(MAX_HEALTH_NAMES[i])
...
local t = math_clamp((tonumber(health) or 0) / maxH, 0, 1)
return C3new(1 - t, t, 0.12)
end
local function drawText(label, pos, text, color, yOffset, baseFontSize, dist)
local anchorPos = V3new(pos.X, pos.Y + (yOffset * OFFSET_STUD_SCALE), pos.Z)
local sPos, onScreen = WTS(anchorPos)
...
label.Center   = true
label.Text     = text
label.Position = V2new(sPos.X, sPos.Y)
label.Color    = color
label.Visible  = true
return sPos
end
...
local sPos, onScreen = WTS(worldPos)
if onScreen and sPos then return V2new(sPos.X, sPos.Y), true end
...
if depth and depth < 0 then sp = V3new(viewport.X - sp.X, viewport.Y - sp.Y, depth) end
local x = math_clamp(sp.X, margin, viewport.X - margin)
local y = math_clamp(sp.Y, margin, viewport.Y - margin)
return V2new(x, y), false
end
end
return nil, false
end
...
for i = #criminalList, 1, -1 do removeEsp(criminalList[i]); tbl_remove(criminalList, i) end
return
end
local tracked = {}
local players = Players:GetPlayers()
local lpUserId = LocalPlayer and LocalPlayer.UserId
for i = 1, #players do
local player = players[i]
if player ~= LocalPlayer and player.UserId ~= lpUserId then
...
for i = #panicList, 1, -1 do removeEsp(panicList[i]); tbl_remove(panicList, i) end
return
end
local tracked = {}
local players = Players:GetPlayers()
...
removeEsp(e)
tbl_remove(panicList, i)
else
tracked[key] = nil
end
end
for _, player in pairs(tracked) do
tbl_insert(panicList, { Player = player, Label = createTextEsp(cfg.panic.fontSize) })
end
end
local function updateDeployableCache()
if not cfg.masterEnabled or not cfg.deployable.enabled then
for i = #deployableList, 1, -1 do removeEsp(deployableList[i]); tbl_remove(deployableList, i) end
return
end
...
if not key or not tracked[key] then
removeEsp(e)
tbl_remove(deployableList, i)
else
tracked[key] = nil
end
end
for _, model in pairs(tracked) do
tbl_insert(deployableList, { Model = model, Label = createTextEsp(cfg.deployable.fontSize) })
end
end
local function updateBountyCache()
if not cfg.masterEnabled or not cfg.bountyVehicle.enabled then
for i = #bountyList, 1, -1 do removeEsp(bountyList[i]); tbl_remove(bountyList, i) end
return
end
...
local tracked  = {}
if vehicles then
local children = vehicles:GetChildren()
for i = 1, #children do
local model = children[i]
if model:IsA("Model") or model:IsA("Folder") then
local key = getKey(model)
...
if not key or not tracked[key] then
removeEsp(e)
tbl_remove(bountyList, i)
else
tracked[key] = nil
end
end
for _, model in pairs(tracked) do
tbl_insert(bountyList, { Model = model, Label = createTextEsp(cfg.bountyVehicle.fontSize) })
end
end
local function updateStolenCache()
if not cfg.masterEnabled or not cfg.stolenVehicle.enabled then
for i = #stolenList, 1, -1 do removeEsp(stolenList[i]); tbl_remove(stolenList, i) end
return
end
...
if not key or not tracked[key] then
removeEsp(e)
tbl_remove(stolenList, i)
else
tracked[key] = nil
end
end
for _, model in pairs(tracked) do
tbl_insert(stolenList, {
Model      = model,
...
for i = #personalList, 1, -1 do removeEsp(personalList[i]); tbl_remove(personalList, i) end
return
end
...
if not key or not tracked[key] then
removeEsp(e)
tbl_remove(personalList, i)
else
tracked[key] = nil
end
end
for _, model in pairs(tracked) do
tbl_insert(personalList, { Model = model, Label = createTextEsp(cfg.personalVehicle.fontSize) })
end
end
...
for i = 1, #personalList do
modelsToCheck[#modelsToCheck + 1] = personalList[i].Model
end
else
local vehicles = Workspace:FindFirstChild("Vehicles")
...
local owner = getOwnerString(model)
if owner and owner == myName then
modelsToCheck[#modelsToCheck + 1] = model
end
end
end
end
for idx = 1, #modelsToCheck do
local model = modelsToCheck[idx]
...
local text = STR_HP_PREFIX .. tostring(math_floor(health + 0.5))
...
local spotlightCacheT  = 0
local SPOTLIGHT_INTERVAL = 0.5
local function getSpotlightNames()
local now = os_clock()
if now - spotlightCacheT < SPOTLIGHT_INTERVAL then return spotlightNames end
spotlightCacheT = now
local idx = 0
local players = Players:GetPlayers()
for i = 1, #players do
...
idx = idx + 1
spotlightNames[idx] = player.Name
end
end
end
for j = idx + 1, #spotlightNames do
spotlightNames[j] = nil
end
return spotlightNames
end
local function updateVisuals()
if not cfg.masterEnabled then
local allLists = {criminalList, panicList, deployableList, bountyList, stolenList, personalList}
for li = 1, 6 do
local list = allLists[li]
for i = 1, #list do hideEntry(list[i]) end
end
for _, st in pairs(vehicleHealthState) do if st.label then st.label.Visible = false end end
...
local sPos, onScreen = WTS(pos)
if onScreen and sPos then
local radius = math_clamp(380 / (dist > 0 and dist or 1), 3, 8)
...
e.Label.Position = V2new(sPos.X, sPos.Y + radius + 4)
...
drawText(e.Label, pos, STR_PANIC, cfg.panic.color, cfg.panic.yOffset, cfg.panic.fontSize, dist)
end
end
end
end
...
local sPos = drawText(e.Label, pos, STR_STOLEN, cfg.stolenVehicle.color, cfg.stolenVehicle.yOffset, cfg.stolenVehicle.fontSize, dist)
...
e.PriceLabel.Text     = STR_DOLLAR .. tostring(price)
e.PriceLabel.Color    = cfg.stolenVehicle.priceColor
e.PriceLabel.Position = V2new(sPos.X, sPos.Y + 16)
e.PriceLabel.Visible  = true
else
if e.PriceLabel then e.PriceLabel.Visible = false end
end
end
end
end
...
local names = getSpotlightNames()
if #names > 0 then
local text = STR_SPOTLIGHT .. tbl_concat(names, ", ")
...
heliSpotlightLabel.Color    = cfg.helicopter.spotlightColor
heliSpotlightLabel.Position = V2new(screenPos.X, screenPos.Y + 16)
...
local function gatherCodeNodes(root, _, out, limit)
...
if key then out[#out + 1] = { inst = child, key = key } end
end
end
...
c.cx = x + sx * 0.5
c.cy = y + sy * 0.5
local sig = math_floor(sx / 3) .. ":" .. math_floor(sy / 3)
local g = groups[sig]
...
local dist = moveMouseToward(gx + gsx * 0.5, gy + (gsy or 18) * 0.5)
local onCell = dist <= math_max((gsx or 20) * 0.35, 10)
...
local cycleTime = math_max(atmState.stepTime * #nodes + 0.3, 0.6)
if now - atmState.lastClick <= cycleTime then return end
atmState.clickedRound = false
end
local delay = math_max((tonumber(cfg.atm.delay) or 50) / 1000, 0.03)
...
local pr = guiRect(pin)
if not pr then return false end
local prx, pry, prw, prh = pr.x, pr.y, pr.w, pr.h
local lr = guiRect(line)
if not lr then return false end
pad = pad or 0
return (pry + pad) < (lr.y + lr.h) and lr.y < (pry + prh - pad)
end
pad = pad or 0
local pTop, pBot = py - ph * 0.5 + pad, py + ph * 0.5 - pad
local lTop, lBot = ly - lh * 0.5, ly + lh * 0.5
...
local now = os_clock()
local delay = math_max((tonumber(cfg.lockpick.delay) or 40) / 1000, 0.03)
if now - lockpickState.lastClick < delay then return end
local pad = math_max(tonumber(cfg.lockpick.tolerance) or 2, 0)
...
local lead = math_max((tonumber(cfg.glasscut.lead) or 0) / 1000, 0)
...
if not barRect then return false end
local barCx = barRect.cx
local zoneRect = guiRect(zone)
if not zoneRect or zoneRect.w < 4 then return false end
local zoneX, zoneW = zoneRect.x, zoneRect.w
if not inViewport(zone, 20) then return false end
local prevX, prevT = crowBarState.barX, crowBarState.barT
crowBarState.barX, crowBarState.barT = barCx, now
if prevX and prevT > 0 then
local dt = now - prevT
if dt > 0 and dt < 0.25 then
crowBarState.frameDt = dt
local v = (barCx - prevX) / dt
...
if math_abs(crowBarState.vel) < 5 then return true end
local latency = math_max((tonumber(cfg.hotwire.latency) or 50) / 1000, 0)
local frameDt = math_min(crowBarState.frameDt or (1 / 60), 0.05)
local marginPct = math_min(math_max(tonumber(cfg.hotwire.margin) or 25, 0), 45) / 100
local inset = zoneW * marginPct * 0.5
local fromX = barCx + crowBarState.vel * latency
local toX = fromX + crowBarState.vel * frameDt
local lo, hi = math_min(fromX, toX), math_max(fromX, toX)
local delay = math_max((tonumber(cfg.hotwire.delay) or 50) / 1000, 0.05)
if now - crowBarState.lastClick >= delay then
if lo <= zoneX + zoneW - inset and hi >= zoneX + inset then
pcall(mouse1click)
...
numHackState.seq[#numHackState.seq + 1] = digit
...
pcall(function()
local descendants = tangle:GetDescendants()
for i = 1, #descendants do
local child = descendants[i]
...
local pair = nil
for wi = 1, #WIRE_COLORS do
local col = WIRE_COLORS[wi]
...
if not grab then return true end
local grabCx, grabCy = grab.cx, grab.cy
local drop = guiRect(dropInst)
if not drop then return true end
local aimX, aimY = drop.x + drop.w * 0.5, drop.y
if wireState.phase == "aim" then
local dist = moveMouseToward(grabCx, grabCy)
if dist <= 10 then
pcall(mouse1press)
wireState.held = true
...
while true do
local menus = getCachedMenus()
if menus and cfg.hotwire.enabled then
local now = os_clock()
...
task.spawn(function()
while true do
pcall(cacheUpdaters[cacheIdx])
cacheIdx = cacheIdx % #cacheUpdaters + 1
task.wait(0.08)
end
end)
task.spawn(function()
...
Lib:Notify("ERLC ESP", "Full ESP & Autos Ready\nUpdate #20 (INSUI) [Perf]", 3, "success")
print("ERLC Full ESP + Hotwire Suite loaded successfully (INSUI, Performance Build)!")
